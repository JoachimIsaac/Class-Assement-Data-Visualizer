...
async def get_input_options(slo: str, measure: str, date: str):
    slo = slo.upper()
    measure = measure.upper()
    ...
    # Check if T1 exists
    if "T1" in dict_db[slo][measure]:
        if date in dict_db[slo][measure]["T1"]:
            options.append("Edit T1")
        else:
            options.append("Add T1")
    
    # Check if T2 exists
    if "T2" in dict_db[slo][measure]:
        if date in dict_db[slo][measure]["T2"]:
            options.append("Edit T2")
        else:
            options.append("Add T2")
